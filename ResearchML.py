...
def getDailyVol_OLD(close,span0=100):
...
def addVerticalBarrier_OLD(tEvents, close, numDays=1):
    t1=close.index.searchsorted(tEvents+pd.Timedelta(days=numDays))
    t1=t1[t1<close.shape[0]]
    t1=(pd.Series(close.index[t1],index=tEvents[:t1.shape[0]]))
    return t1

def getT1(close,tEvents,span=pd.Timedelta(days=1)):
    t1=close.index.searchsorted(tEvents+span)
    t1=t1[t1<close.shape[0]]
    return pd.Series(close.index[t1],index=tEvents[:t1.shape[0]]).rename("t1") # Nans at end
...
def getIndMatrix(barIx,t1):
    ...
    indM=pd.DataFrame(0,index=barIx,columns=range(t1.shape[0]))#,dtype=bool)
...
def getTimeDecay(tW,clfLastW=1.):
    ...
    # newst observations gets weight=1, oldest  weight=clfLastW
...
def fracDiff_FFD(series,d,thres=1e-5):
    ...
    for iloc1 in range(width,seriesF.shape[0]):
        ...
        df_.loc[loc1]=np.dot(w.T, seriesF.loc[loc0:loc1])[0]
    return df_

def findMinD(series):
    from statsmodels.tsa.stattools import adfuller
    for d in np.linspace(0,1,11):
        series_=fracDiff_FFD(series,d).copy(deep=True)
        adf=adfuller(series_,maxlag=1,regression='c',autolag=None)
        if adf[1]<0.05:
            if d == 0:
                print(f"{series.name} achieved stationarity at: {d}")
                return series_
            else:
                for a in np.linspace(d-0.09,d,10):
                    series_=fracDiff_FFD(series,a)
                    adf=adfuller(series_,maxlag=1,regression='c',autolag=None)
                    print(f"{series.name} achieved stationarity at: {a}")
                    return series_

def getDifFeaturesMP(df):
    import multiprocessing as mp
    with mp.Pool(mp.cpu_count()-1) as p:
        res = p.map(findMinD,[df[c] for c in list(df)])
    return pd.concat(res,axis=1)

# =======================================================
# finding the min. D value that passes ADF test

def plotMinFFD_OLD(df0,col={"close":"cVb"},maxLag=1,thres=1e-5):
    # pg. 85
    from statsmodels.tsa.stattools import adfuller
    import matplotlib.pyplot as plt
    ...
    for d in np.linspace(0,1,11):
        df1=np.log(df0[[col["close"]]]).resample('1D').last() # downcast to daily obs
        df2=fracDiff_FFD_OLD(df1,d,thres=thres)
        corr=np.corrcoef(df1.loc[df2.index,col["close"]],df2[col["close"]])[0,1]
        df2=adfuller(df2[col["close"]],maxlag=maxLag,regression='c',autolag=None)
...
def cvScore(clf,X,y,sample_weight,scoring='neg_log_loss',
    ...
    for train,test in cvGen.split(X=X):
        print(sample_weight.iloc[idx[train]].values)
...
def plotFeatImportance(pathOut,imp,oob,oos,method,tag=0,simNum=0,**kargs):
    ...
    if method=='MDI':
        ...
        mpl.axvlifffffffffddddne(1./imp.shape[0],lw=1.,color='r',ls='dotted')
...
def printScore(m,xTrain,yTrain,xTest,yTest, xValid=None, yValid=None):
    res = {
        "TrainPredict": rmse(m.predict(xTrain),yTrain),
        "TestPredict": rmse(m.predict(xTest),yTest),
        "TrainScore": m.score(xTrain,yTrain),
        "TestScore": m.score(xTest,yTest),
    }
    if hasattr(m, "oob_score_"): res["oobScore"] = m.oob_score_
    print(res)
    print(m)


def printHitrate(yPred):
    res = []
    for t in yPred["target"].unique():
        res.append(yPred[yPred["target"]==t][yPred["target"]==yPred["pred"]].shape[0]/yPred[yPred["target"]==t].shape[0])
    print(dict(zip(yPred["target"].unique().astype(int),res)))
...
def printFeatImp(m, xTrain):
    fi = rf_feat_importance(m, xTrain)
    p = fi.plot("cols", "imp", figsize=(10,30),legend=False, kind="barh")
...
#=================================
# Depreciated
# def applyTimeElapsedAfterEventSP(df,tEvent,y,interval="m"):
#     baseDate=0
#     def applyT(row):
#         nonlocal baseDate,tEvent,y,interval
#         if np.datetime64(row) in tEvent: baseDate=row
#         try: return ((row-baseDate) / np.timedelta64(1, interval))**y
#         except(ValueError,ZeroDivisionError): return 0
#     return df.apply(applyT)
#=================================

def timeElapsedAfterEventVectorized(idx,tEvents,y,td=timedelta(minutes=1)):
    df0=pd.DataFrame(index=idx)
    df0.loc[tEvents,"base"]=tEvents
    df0=df0.fillna(method="ffill")
    df0["tea"] = np.where((df0.index-df0.base)/td==.0,1,((df0.index-df0.base)/td)**y)
    return df0["tea"]

def applyLocalize(molecule, dtSeries, tz="America/New_York"):
    import pytz
    return dtSeries.loc[molecule].apply(pytz.timezone(tz).localize)

def splitIntoTrainTestValid(df,y1,y2, target="target", dateCol="date", save=False, paths=None):
    '''returns (xTrain, yTrain, xTest, yTest, xValid, yValid) DataFrames'''
    xTrain = df[df[dateCol].dt.year < y1].copy().drop(dateCol,1)
    xTest = df[df[dateCol].dt.year == y1].copy().drop(dateCol,1)
    xValid = df[df[dateCol].dt.year == y2].copy().drop(dateCol,1)
    if save:
        xTrain.reset_index(drop=True).to_feather(paths[0])
        xTest.reset_index(drop=True).to_feather(paths[1])
        xValid.reset_index(drop=True).to_feather(paths[2])
    return xTrain.drop(target,1), xTrain[target], xTest.drop(target,1),xTest[target], xValid.drop(target,1),xValid[target]


class Sampling:
    @staticmethod
    def VPIN(b,s,v,n=1000):
        return abs(b-s).rolling(window=n,min_periods=1).sum() / v.rolling(window=n,min_periods=1).sum()
